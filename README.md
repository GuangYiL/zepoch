...
`zepoch` is an explicit time library for Zig 0.17.0-dev.1387+01b60634c. It
separates absolute instants, wall time, fixed-offset views, and monotonic clocks
into distinct types. External capabilities enter through Zig 0.17 `std.Io` and
`std.Tz`; the library never guesses a time zone, reads global process state, or
hides allocation.
...
- `Instant.now(io)` and `MonotonicInstant.now(io, clock)` receive `std.Io` explicitly.
...
The `zig-0.17` branch requires Zig `0.17.0-dev.1387+01b60634c` on `PATH`:
