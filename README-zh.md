...
`zepoch` 是面向 Zig 0.15.2 的显式时间库。它把绝对时间、墙上时间、固定偏移
视图和单调时间拆成不同类型。外部时钟能力通过本库可注入的 `Clock` 接口传入，
TZif 解析使用 `std.Tz`。库不会猜测时区、读取进程环境或隐藏分配。

## 版本支持

持续维护范围只包含 Zig 当前稳定发行版、前一个发行系列和后一个开发系列：

| 分支 | Zig 版本 | 状态 |
| --- | --- | --- |
| `zig-0.15` | 0.15.2 | 前一发行版，持续维护 |
| `main` | 0.16.0 | 当前稳定发行版，持续维护 |
| `zig-0.17` | 0.17 开发快照 | 后一发行版，预览维护 |

新的 Zig 稳定版发布并推动这三个版本的支持窗口后，超出窗口的旧版本分支仍会
保留，以便复现历史构建，但不再接收兼容性适配、缺陷修复或其他维护。
...
- `Instant.now(clock)` 和 `MonotonicInstant.now(clock)` 显式接收 `Clock` 能力；
  生产代码可传入系统时钟，测试可注入固定时钟。
- 平台无法满足 `realtime`、`awake` 或 `boot` 的准确语义时，系统时钟返回
  `error.ClockUnavailable`，不会用其他时钟静默代替。
...
`Instant.now(system_realtime_clock)` 通过显式能力读取平台的 POSIX 风格实时时钟，
再规范化为 `i64 unix_seconds` 和 `[0, 1_000_000_000)` 范围的
`u32 nanosecond`。负值使用 floor 除法与非负余数，因此 `-1 ns` 表示为
`-1 s + 999_999_999 ns`。

`Instant` 遵循 POSIX 时间：使用 Unix 纪元，不表示插入的闰秒。
