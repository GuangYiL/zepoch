...
pub const Instant = struct {
    ...
    pub fn now(io: std.Io) error{Overflow}!Instant {
        return fromUnixNanoseconds(std.Io.Clock.now(.real, io).nanoseconds);
    }
...
test "Instant.now reads the explicit POSIX real clock" {
    const current = try Instant.now(testing.io);
    try testing.expect(current.nanosecond < nanoseconds_per_second);
}
